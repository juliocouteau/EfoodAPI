...
const CartContainer = styled.div`
  ...
  z-index: 200; // Aumentado para ficar acima da modal se necessário
...
const Sidebar = styled.aside`
  ...
  color: #FFEBD9;
  overflow-y: auto; // Garante scroll se houver muitos itens
...
export const Cart = ({ isOpen, closeCart, items, removeItem, clearCart }) => {
  const [step, setStep] = useState('cart')

  // CÁLCULO REAL: Soma o preço de cada item que está no array
  const total = items.reduce((acc, item) => acc + (item.preco || 0), 0)
  ...
  return (
    <CartContainer isOpen={isOpen}>
      <Overlay onClick={handleOnlyClose} />
      ...
      <Sidebar>
        {step === 'cart' && (
          <>
            ...
            {items.length > 0 && (
              <>
                <div style={{display: 'flex', justifyContent: 'space-between', marginTop: '24px', marginBottom: '16px'}}>
            ...
            <Button onClick={handleOnlyClose}>Voltar para a loja</Button>
          </>
        )}
        ...
        {step === 'delivery' && (
          <FormContainer>
            ...
            <Button style={{marginTop: '16px'}} onClick={() => setStep('payment')}>Continuar com o pagamento</Button>
        ...
        {step === 'payment' && (
          <FormContainer>
            ...
            <Row>
              <div style={{flex: 2}}><label>Número do cartão</label><input type="text" /></div>
              <div style={{flex: 1}}><label>CVV</label><input type="text" /></div>
            </Row>
            <Row>
              <div><label>Mês de vencimento</label><input type="text" /></div>
              <div><label>Ano de vencimento</label><input type="text" /></div>
            </Row>
            <Button style={{marginTop: '16px'}} onClick={() => setStep('success')}>Finalizar pagamento</Button>
        ...
        {step === 'success' && (
          <>
            ...
            <Button onClick={handleFinishAndClear}>Concluir</Button>
          </>
        )}
      </Sidebar>
    </CartContainer>
